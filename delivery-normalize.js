(()=>{
'use strict';
const DENSITY={aluminum:2.71,stainless:7.93,steel:7.85,brass:8.50,copper:8.96};
...
function isKyushuOdo(s){return /九州黄銅/.test(String(s||'').normalize('NFKC').replace(/[\s　]/g,''));}
...
function normalizeCard(card){if(card.dataset.m2Normalized==='1')return;const get=k=>card.querySelector(`[data-k="${k}"]`);const date=get('delivery_date');if(date&&/^20(0[1-9]|1\d)-/.test(date.value))date.value=fixDate(date.value);
 const unit=get('unit'),price=get('unit_price'),qty=get('quantity'),thEl=get('thickness_mm'),spec=get('spec'),name=get('name'),form=get('purchase_form'),notes=get('notes'),supplier=get('supplier');if(!unit||!price)return;
 ...
 if(u==='S'){
   ...
   const den=densityFor(text);if(th&&den){const kgPerM2=th*den;newPrice=p*kgPerM2;newQty=q==null?null:q/kgPerM2;const kyushu=isKyushuOdo(supplier?.value);reason=`元単価 ${p.toLocaleString()}円/${u==='K'?'K':'kg'}（${kyushu?'九州黄銅では K=kg と固定':'kg と解釈'}） → ${Math.round(newPrice).toLocaleString()}円/㎡（厚み${th}mm・密度${den}）`;}
 }
