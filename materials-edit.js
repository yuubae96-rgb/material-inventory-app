...
(function(){
  ...
  // iPhone Safariが「前回のスクロール位置」だけ先に復元すると、上半分が空白に見える。
  // この画面は開いた時は必ず先頭から表示する。
  try{if('scrollRestoration' in history)history.scrollRestoration='manual'}catch(e){}
  const resetInitialScroll=()=>{if(!location.hash)requestAnimationFrame(()=>window.scrollTo(0,0))};
  window.addEventListener('pageshow',e=>{if(e.persisted||performance.now()<2500)resetInitialScroll()},{passive:true});
  ...
  function ensureStyles(){
    ...
    const s=document.createElement('style');s.id='materialEditStyles';s.textContent=`
      ...
      .material-edit-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}.material-edit-box label{display:block;font-size:11px;font-weight:800;margin-bottom:4px;color:#4b5563}
  ...
  async function fetchMaterials(){
    ...
    if(error){console.error('edit materials load',error);return activeMaterials}
    activeMaterials=data||[];return activeMaterials;
  }
  ...
  function attachButtons(){
    const rows=[...document.querySelectorAll('#mList .material-list-item')],shown=filteredMaterials();
    rows.forEach((row,i)=>{const m=shown[i];if(!m)return;let btn=row.querySelector('.material-edit-btn');if(!btn){btn=document.createElement('button');btn.type='button';btn.className='material-edit-btn';btn.textContent='編集';row.appendChild(btn)}btn.dataset.id=m.id});
  }
  ...
  function openEditor(row,id){
    ...
    // smooth scrollはiPhoneで入力反応を止めることがあるので使わない。
    box.scrollIntoView({behavior:'auto',block:'nearest'});
    latestPrice(id).then(p=>{if(editingId!==id||!box.isConnected)return;const pi=box.querySelector('#me_price'),di=box.querySelector('#me_price_date');if(pi){pi.value=p?.price??'';pi.placeholder=''}if(di)di.value=p?.effective_from||today()}).catch(e=>console.error('price load',e));
  }
  ...
  async function saveEdit(){
    ...
    const saveBtn=document.querySelector('.material-edit-save');if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='保存中…'}
    const {error}=await supabaseClient.from('materials').update(changes).eq('id',editingId);if(error){if(saveBtn){saveBtn.disabled=false;saveBtn.textContent='保存'}alert('材料の修正に失敗しました');return}
  ...
  function showTab(btn){const tab=btn.dataset.tab;if(!tab)return;const panel=document.getElementById('mp-'+tab);if(!panel)return;document.querySelectorAll('.material-tab').forEach(x=>x.classList.toggle('active',x===btn));document.querySelectorAll('.material-panel').forEach(x=>{const on=x===panel;x.classList.toggle('active',on);x.style.display=on?'block':'none'})}
  ...
  async function init(){
    ensureStyles();hookTabs();ensureSquareMeterUnit();resetInitialScroll();
    // 編集用に材料マスターだけ1回取得。タブを押すたびの全テーブル再読込は廃止。
    await fetchMaterials();scheduleAttach();
    const list=document.getElementById('mList');if(list)new MutationObserver(scheduleAttach).observe(list,{childList:true});
    document.getElementById('mSearch')?.addEventListener('input',scheduleAttach,{passive:true});
    // 編集ボタンは各行にonclickを大量設定せず、一覧全体で1本だけ受ける。
    list?.addEventListener('click',e=>{const btn=e.target.closest?.('.material-edit-btn');if(!btn)return;e.preventDefault();e.stopPropagation();const row=btn.closest('.material-list-item');if(row)openEditor(row,btn.dataset.id)},true);
    setTimeout(loadStocktake,500);
    setTimeout(hookTabs,700);
  }
