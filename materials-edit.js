...
(function(){
  ...
  function ensureStyles(){
    ...
    s.textContent=`
      .material-tab{position:relative;opacity:.68!important;filter:saturate(.72)}
      .material-tab.active{opacity:1!important;filter:none!important;outline:4px solid #f59e0b!important;outline-offset:2px!important;box-shadow:0 0 0 2px #fff inset!important}
      .material-tab.active::before{content:'✓';display:inline-block;margin-right:6px;font-weight:1000}
  ...
  async function saveEdit(){
    ...
    const changes={
      name,
      spec:document.getElementById('me_spec').value.trim()||null,
      thickness_mm:thick===''?null:Number(thick),
      purchase_form:document.getElementById('me_purchase').value.trim()||null,
      supplier:document.getElementById('me_supplier').value.trim()||null,
      reorder_point:Number(document.getElementById('me_reorder').value)||0,
      storage_location:document.getElementById('me_location').value.trim()||null,
    ...
    if(Number.isFinite(price)&&price>0){
      const old=await latestPrice(editingId);
      const date=document.getElementById('me_price_date').value||today();
  ...
  function markActiveTab(){
    document.querySelectorAll('.material-tab').forEach(btn=>{
      btn.addEventListener('click',()=>setTimeout(()=>{
        document.querySelectorAll('.material-tab').forEach(x=>x.classList.toggle('active',x===btn));
      },0));
    });
  }
