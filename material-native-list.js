(()=>{
'use strict';
if(window.__materialNativeListInstalled)return;window.__materialNativeListInstalled=true;
const openIds=new Set();
const oldRenderList=window.renderList;
function cleanPrice(p,m){if(!p)return '単価未登録';const unit=p.price_basis==='kg'?'kg':(m.stock_unit||'');return `${fmt(p.price)}円 / ${unit}`}
function thicknessOf(m){if(m.thickness_mm!==null&&m.thickness_mm!==undefined&&m.thickness_mm!=='')return `${m.thickness_mm}mm`;const s=`${m.name||''} ${m.spec||''} ${m.purchase_form||''}`;let x=s.match(/([0-9]+(?:\.[0-9]+)?)\s*(?:μm|um|μ)/i);return x?`${x[1]}μm`:'—'}
function renderNativeList(){
  ...
  $('mList').innerHTML=list.length?list.map(m=>{
    const s=stockFor(m.id),p=latestPriceFor(m.id),alert=Number(m.reorder_point)>0&&s<=Number(m.reorder_point),isOpen=openIds.has(String(m.id));
    ...
    return `<div class="material-list-item material-native-row${isOpen?' material-native-open':''}" data-material-id="${esc(m.id)}">
      <button type="button" class="material-native-summary" data-native-id="${esc(m.id)}">
        <span class="mnl-name">${esc(m.name)}</span>
        <span class="mnl-thick">${esc(thicknessOf(m))}</span>
        <span class="mnl-price">${esc(cleanPrice(p,m))}</span>
        <span class="mnl-arrow">${isOpen?'▲':'▼'}</span>
      </button>
