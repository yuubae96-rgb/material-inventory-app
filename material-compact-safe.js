...
function build(row){if(row.querySelector(':scope > .material-compact-summary'))return;const id=getId(row);if(!id)return;const name=(row.querySelector('.material-name')?.textContent||'名称なし').trim();const price=cleanPrice(row.querySelector('.material-price')?.textContent);const summary=document.createElement('button');summary.type='button';summary.className='material-compact-summary';summary.innerHTML=`<span class="mcs-name">${esc(name)}</span><span class="mcs-thick">${esc(getThickness(row))}</span><span class="mcs-price">${esc(price)}</span><span class="mcs-arrow">▼</span>`;summary.addEventListener('click',()=>{const open=!expanded.has(id);if(open)expanded.add(id);else expanded.delete(id);row.classList.toggle('material-compact-open',open);summary.querySelector('.mcs-arrow').textContent=open?'▲':'▼'});row.insertBefore(summary,row.firstChild);row.classList.add('material-compact-safe')}
