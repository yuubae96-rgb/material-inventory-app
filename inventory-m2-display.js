...
function densityName(t){t=String(t||'').normalize('NFKC').toLowerCase();if(/a1\d{3}|a1050|a1070|アルミ/.test(t))return 'アルミ';if(/sus|ステンレス/.test(t))return 'ステンレス';if(/真鍮|brass|c2[678]\d{2}/.test(t))return '真鍮';if(/銅|copper|c1100/.test(t))return '銅';if(/鉄|steel|spcc|ss400/.test(t))return '鉄';return '材料'}
...
function normalizeRow(row){
 ...
 const ptxt=price.textContent||'';const pm=ptxt.replace(/,/g,'').match(/最新単価\s*([\d.]+)円\s*\/\s*([KS㎡m²]+)/i);
 if(pm){
   const p=+pm[1],u=pm[2].trim().toUpperCase();let m2=null,detail='',calc='';
   if(u==='K'&&dm?.th&&den){
     const kgPerM2=dm.th*den;m2=p*kgPerM2;
     detail=`元 ${fmt(p)}円/kg`;
     calc=`計算：${fmt(dm.th)}mm × ${fmt(den)}kg/㎡/mm ＝ ${fmt(kgPerM2)}kg/㎡　→　${fmt(p)}円/kg × ${fmt(kgPerM2)}kg/㎡ ＝ ${fmt(m2)}円/㎡`;
   }else if(u==='S'&&dm?.w&&dm?.l){
     const wm=dm.w/1000,lm=dm.l/1000,area=wm*lm;
     if(area>0){m2=p/area;detail=`元 ${fmt(p)}円/シート`;calc=`計算：${fmt(dm.w)}mm × ${fmt(dm.l)}mm ＝ ${fmt(wm)}m × ${fmt(lm)}m ＝ ${fmt(area)}㎡/シート　→　${fmt(p)}円 ÷ ${fmt(area)}㎡ ＝ ${fmt(m2)}円/㎡`;}
   }
   if(m2!=null){
     const date=(ptxt.match(/\(([^)]+)\)/)||[])[1]||'';
     price.textContent=`最新単価 ${fmt(m2)}円 / ㎡${date?` (${fixDateText(date)})`:''}`;
     let note=row.querySelector('.m2-display-note');if(!note){note=document.createElement('div');note.className='m2-display-note';note.style.cssText='font-size:12px;color:#287a3d;font-weight:800;margin-top:4px;line-height:1.55';price.insertAdjacentElement('afterend',note)}
     note.innerHTML=`㎡換算表示（${detail}）<br><span style="font-weight:600;color:#475569">${calc}</span>`;changed=true;
